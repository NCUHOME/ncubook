...
export function ImageBlock({ block, asset, resolvePageRoute }: { block: ExtractBlock<"image">; asset: Asset | null; resolvePageRoute: (pageId: string) => string }) {
  ...
  return <figure id={block.anchor}><img className="h-auto w-full" src={asset.publicUrl} alt={asset.alt ?? ""} />{block.caption ? <figcaption className="mt-s2 text-caption leading-ui text-muted"><RichText value={block.caption} resolvePageRoute={resolvePageRoute} /></figcaption> : null}</figure>;
}
