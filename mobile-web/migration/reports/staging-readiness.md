...
1. The connected Notion app does not expose a server runtime token. The authoritative publisher dry-run still requires a read-only Notion Integration token configured outside Git.
2. The Supabase connector cannot export a service-role runtime credential. Deployment still needs that secret configured outside Git.
