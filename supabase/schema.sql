-- ============================================================================
-- 此间 (NCU Book) - 生产数据库 SQL Schema 契约文件 (supabase/schema.sql)
-- 包含：扩展插件、版本化发布表、检索向量索引、事务 RPC 函数与 RLS 行级安全策略
-- ============================================================================

-- 1. PostgreSQL 扩展插件 (加密、向量计算与三元组模糊匹配)
...
-- RPC 执行权限管控：严禁匿名/普通前端账户调用管理级 RPC
...
-- 7. 行级安全策略 (Row-Level Security - RLS Policies)
-- 开启只读防修改保护
...
-- 前端及匿名用户仅可 SELECT 当前在线（Pointer 指向）的版本数据
