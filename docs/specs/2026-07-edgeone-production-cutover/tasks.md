...
- [ ] E1.2 为 RPC 增加最低词法相关度过滤，新增 answerable/unanswerable 中文检索评测并验证 recall@8/空召回目标。
...
- [ ] E2.2 增加真实 DeepSeek smoke：answerable grounded success、unanswerable abstention、citation anchor、active contentVersion 和 p95；通过前保持 shadow。
...
- [ ] E3.3 保存并验证旧 Docusaurus revision/build 配置；关闭任意分支自动 preview，配置 Gate E 环境矩阵并创建固定 revision 的受控 preview。
- [ ] E3.4 在 preview 验证首页、板块、文档、搜索、AI、citation、资源和 360/390/430px 交互。
- [ ] E3.5 以临时域名执行并计时新版→旧 Docusaurus→同一新版回滚演练。
- [ ] E3.6 将验证通过的 revision 部署为 production，并保持 `book.ncuos.com`；记录 deployment、revision 与独立回滚构建入口。
