...
EdgeOne 直接构建仓库根目录的 Next.js 学生端，由其运行时承载 Server Components 和 Route Handlers。页面、搜索与问答保持同源；所有服务端密钥由 EdgeOne 环境变量注入。
...
- 项目根目录：`.`；原 `mobile-web/` 内容在合并前提升到仓库根目录。
...
旧 Docusaurus 不再占用仓库根构建入口。其可恢复资产为切换前固定 revision `b0e4de1d4a6aaa8979777b4ded21e5d45d4c4088`、对应根目录构建命令 `pnpm build` 和产物目录 `build/`；无需在当前工作树保留一套会与 Next.js 根项目冲突的重复依赖。
