...
EdgeOne 控制台为 `/api/ask` 增加平台级速率限制/WAF 规则，作为多实例环境的外层费用保护；应用内每 IP 每分钟限制继续作为第二层。preview 通过突发请求验证 429，且普通搜索、文档和静态资源不受该规则影响。

## 可重复的性能验证

- 问题集固定为 5 个可回答问题，覆盖首页提问与文档上下文提问。
- 先执行 1 次不计入统计的 warm-up，再执行 20 次端到端 `/api/ask` 请求；从客户端发出请求到收到完整 JSON 响应为测量边界，包含检索与模型生成。
- 20 次中所有 429、超时和 5xx 计为失败，不能从延迟样本中删除；p95 必须不超过 5 秒。
- 另执行 3 次相隔足够时间的冷启动请求并单独记录，单次不得超过 provider timeout 8 秒；冷启动不混入 warm p95。
