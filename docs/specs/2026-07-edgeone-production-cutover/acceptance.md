...
- [ ] 请求显式设置 `thinking.type=disabled`；真实 answerable smoke p95 不超过 5 秒，超时/429/5xx fail closed。
- [ ] 浏览器 bundle、Git 历史和响应中不存在 DeepSeek Key。
- [ ] 对话中出现的旧 Key 在远端部署前已废止，远端只使用替换 Key。
- [ ] 无 embedding 配置时，answerable 问题 recall@8 不低于 80% 并能形成 grounded citation；unanswerable 空召回率 100% 且返回 insufficient。
- [ ] 关键词搜索没有调用 DeepSeek。
...
- [ ] 已保存可独立重新部署的旧 Docusaurus revision/build 配置，并完成新版→旧版→新版计时演练；回滚不修改 Supabase 内容指针。
